	...
	def handle(self, *args, **kwargs):
		...
		for line in in_file:
			...
			if "Portland" in parsed_line[7]:
				...
				print ("length = "+repr(len(update_priest)))
				print (repr(update_priest))
									
				count = len(update_priest)
				count -= 1
				if count >= 1:
					print ("deleting dup ")
				while count > 0:
					update_priest.delete()
					count -= 1
				if count >= 1:
					priest = update_priest[0]
					if insert_arch:
						priest.archdiocese = arch_obj
					else:
						priest.diocese = dio_obj
					#priest.save()
				elif count == 0:
